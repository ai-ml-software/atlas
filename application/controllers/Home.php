...
class Home extends CI_Controller
{
    ...
    public function profile($param1 = "")
    {
        ...
        // Without a sub page nothing sets page_name and the view include
        // fails with a 500. The profile itself is the sensible default.
        if ($param1 == "") {
            $param1 = 'user_profile';
        }
    ...
    public function update_profile($param1 = "", $is_profile_page = false)
    {
        if ($param1 == 'update_basics') {
            ...
            if (isset($_FILES['user_image']) && $_FILES['user_image']['name'] != "") {
                unlink('uploads/user_image/' . $this->db->get_where('users', ['id' => $this->session->userdata('user_id')])->row('image') . '.jpg');
    ...
    public function toggleWishlistItems($course_id = "", $identifier = "")
    {
        if ($this->session->userdata('user_login') != 1) {
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "")
    {
        ...
        if ($course_details['course_type'] == 'general') {
            ...
            if ($sections->num_rows() > 0) {
                ...
                if ($lesson_id == "") {
                    ...
                    $section_id              = $this->db->get_where('lesson', ['id' => $lesson_id])->row()->section_id;
        ...
        $announcements = $this->db->select('announcements.*')->from('announcements')->join(
            'announcement_courses',
            'announcement_courses.announcement_id = announcements.id'
        )->where('announcement_courses.course_id', $course_id)
        ->where('announcements.status', 1)
        ->order_by('announcements.is_pinned', 'DESC')
        ->order_by('announcements.id', 'DESC')
        ->get()
        ->result_array();

        $page_data['announcements'] = $announcements;
        $page_data['course_id']             = $course_id;
    ...
    public function pdf_canvas($course_id = "", $lesson_id = "", $bundle_id = "")
    {
    ...
    public function compare()
    {
        ...
        $page_data['course_1_details'] = $course_id_1 ? $this->crud_model->get_course_by_id($course_id_1)->row_array() : [];
        $page_data['course_2_details'] = $course_id_2 ? $this->crud_model->get_course_by_id($course_id_2)->row_array() : [];
        $page_data['course_3_details'] = $course_id_3 ? $this->crud_model->get_course_by_id($course_id_3)->row_array() : [];
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    /** COURSE COMPARE ENDS */
    ...
    public function course_purchase($auth_token = '', $course_id = '')
    {
        ...
        if (empty($auth_token) || $auth_token == "null") {
            ...
            if ($logged_in_user_details['user_id'] > 0) {
                ...
                $credential = ['id' => $logged_in_user_details['user_id'], 'status' => 1, 'role_id' => 2];
    ...
    public function play_lesson($lesson_id = "", $is_preview = "")
    {
    ...
    public function closed_back_to_mobile_ber()
    {
    ...
    {
        $page_data['quiz_results']   = $this->db->get_where('quiz_results', ['quiz_result_id' => $quiz_result_id]);
        $page_data['lesson_details'] = $this->crud_model->get_lessons('lesson', $page_data['quiz_results']->row('quiz_id'))->row_array();
        $page_data['quiz_questions'] = $this->db->get_where('question', ['quiz_id' => $page_data['quiz_results']->row('quiz_id')]);
    ...
    public function check_gift_user()
    {
    ...
    public function gift_to_user_id()
    {
        $this->session->unset_userdata('send_gift_to_id');
        $status  = 0;
    ...
    public function subscribe_to_our_newsletter()
    {
    ...
    public function get_compare_course_select2($course_1 = "0", $course_2 = "0", $course_3 = "0")
    {
    ...
    public function dark_and_light_mode()
    {
    ...
    public function coupon_offer_100_percent()
    {
        $cart_items     = $this->session->userdata('cart_items');
    ...
    public function course_playing_page_layout()
    {
    ...
    public function become_an_instructor()
    {
        ...
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST) && count($_POST) > 0):
                $accepted_ext = ['doc', 'docs', 'pdf', 'txt', 'png', 'jpg', 'jpeg'];
    ...
    public function lesson_mobile_web_view_get($lesson_id = "")
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function offline_video_for_mobile_app($lesson_id = "")
    {
    ...
    public function posts($param1 = "", $param2 = "", $param3 = "")
    {
        ...
        if ($param1 == 'add') {
            ...
            $response = $this->crud_model->add_post();

            if ($response) {
                $this->session->set_flashdata('flash_message', get_phrase('Post created successfully'));
            } else {
                $this->session->set_flashdata('error_message', get_phrase('Something went wrong'));
            }

            redirect(site_url('home/posts'), 'refresh');
        }
        ...
        if ($param1 == 'delete') {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('Post deleted successfully'));

            if ($param3 == 'posts') {
                redirect(site_url('home/posts'), 'refresh');
            }

            redirect(site_url('home/my_posts'), 'refresh');
        }

        $search = $this->input->get('search', true);

        $page_data['posts'] = $this->crud_model->get_posts(null, $search);

        $page_data['search']     = $search;
        $page_data['page_name']  = "community_index";
        $page_data['page_title'] = site_phrase('Posts');
    ...
    public function toggle_like($post_id)
    {
        ...
        $response = $this->crud_model->toggle_like(
            $post_id,
    ...
    public function shared_post($id = '')
    {
        ...
        if (empty($page_data['post'])) {
            show_404();
        }

        $page_data['popular_posts'] = $this->db
            ->from('community_posts')
            ->where('id !=', $id)
            ->order_by('id', 'DESC')
            ->get()
        ...
        $page_data['page_name']  = 'community_shared_post';
        $page_data['page_title'] = get_phrase('Shared Post');
    ...
    public function add_comment($post_id)
    {
        ...
        $response = $this->crud_model->add_comment(
            $post_id,
            $this->session->userdata('user_id')
        );
    ...
    public function toggle_save($post_id)
    {
        if (! $this->session->userdata('user_id')) {
        ...
        $response = $this->crud_model->toggle_save_post(
            $post_id,
            $this->session->userdata('user_id')
        );
    ...
    public function my_posts()
    {
        ...
        $page_data['posts'] = $user_id
            ? $this->crud_model->get_posts($user_id)
            : [];

        $page_data['page_name']  = "community_my_posts";
        $page_data['page_title'] = site_phrase('My Posts');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function saved_posts()
    {
        ...
        $page_data['posts'] = $user_id
            ? $this->crud_model->get_saved_posts($user_id)
            : [];

        $page_data['page_name']  = "community_saved_posts";
        $page_data['page_title'] = site_phrase('Saved Posts');

        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function remove_saved_post($post_id)
    {
        $user_id = $this->session->userdata('user_id');
        ...
        $this->session->set_flashdata('flash_message', get_phrase('Bookmark removed successfully'));

        redirect(site_url('home/saved_posts'), 'refresh');
    }

    public function download_announcement_file($file_name = '')
    {
        $this->load->helper('download');

        $path = FCPATH . 'uploads/announcements/' . $file_name;
