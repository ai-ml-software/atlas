...
class Admin extends CI_Controller
{
    ...
    public function course_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_course') {
            ...
            // Anything else reached this method and fell through to a blank
            // 200 page, which reads as "the admin panel is broken". Send the
            // visitor back to the course list instead.
            redirect(site_url('admin/courses'), 'refresh');
        }
    }
    ...
    public function lessons($course_id = "", $param1 = "", $param2 = "")
    {
        ...
        // Lessons are managed inside the course editor; there is no
        // standalone backend/admin/lessons.php view, so rendering one here
        // produced an empty panel. Send the request where the work happens,
        // which is what the delete branch above already does.
        if ($course_id === "") {
            redirect(site_url('admin/courses'), 'refresh');
        }
        redirect(site_url('admin/course_form/course_edit/' . $course_id), 'refresh');
    ...
    {
        // The navigation hides this behind addon_status('course_ai'), but the
        // URL is still reachable and the addon ships no view, so without this
        // guard the page renders as a broken include.
        if (!addon_status('course_ai')) {
            $this->session->set_flashdata('error_message', get_phrase('this_addon_is_not_installed'));
            redirect(site_url('admin/system_settings'), 'refresh');
        }
        if ($param1 == "update") {
...
// Custom Field End
    ...
    // Home page builder
    /**
     * Which of the two public front ends the site root serves.
     *
     * Kept next to the Home Page Builder because that is where its absence
     * was felt: an administrator would build a home page, activate a theme,
     * see no change at the site root, and have no way to discover that the
     * root was serving the other front end.
     */
    public function root_frontend()
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $choice = $this->input->post('root_frontend') === 'lms' ? 'lms' : 'academy';
        $exists = $this->db->where('key', 'root_frontend')->count_all_results('frontend_settings');
        if ($exists) {
            $this->db->where('key', 'root_frontend')->update('frontend_settings', array('value' => $choice));
        } else {
            $this->db->insert('frontend_settings', array('key' => 'root_frontend', 'value' => $choice));
        }
