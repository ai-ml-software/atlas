...
class Crud_model extends CI_Model
{
    ...
    public function update_frontend_settings()
    {
        ...
        $this->update_chrome_settings();
    }

    /**
     * Site header and footer copy for the academy front end.
     *
     * Kept as a whitelisted loop rather than another twelve hand-written
     * blocks above: the list is the contract, a key absent from it is never
     * written, and adding a field is one line. Only keys POSTed on this
     * request are touched, so saving any other tab on the settings screen
     * cannot blank these.
     */
    private function update_chrome_settings()
    {
        $keys = array(
            'ha_rail_note_en', 'ha_rail_note_ar',
        ...
        foreach ($keys as $key) {
            ...
            // Stored raw and escaped at render; the address is the one field
            // that carries newlines, and html_escape here would defeat nl2br.
            $value = trim((string) $posted);

            $exists = $this->db->where('key', $key)->count_all_results('frontend_settings');
            if ($exists) {
                $this->db->where('key', $key)->update('frontend_settings', array('value' => $value));
            } else {
                $this->db->insert('frontend_settings', array('key' => $key, 'value' => $value));
            }
        }
    }
