<script type="text/javascript">
    function changeTitleOfImageUploader(photoElem) {
        ...
        <?php
        // upload_max_filesize carries a K, M or G suffix. Stripping only 'm'
        // left "2g" for a 2G limit, which PHP 8 warns about and evaluates to
        // zero, so every upload was reported as too large.
        $upload_max_filesize_raw = trim(strtolower((string) ini_get('upload_max_filesize')));
        $upload_max_filesize_unit = substr($upload_max_filesize_raw, -1);
        $upload_max_filesize_num  = (float) $upload_max_filesize_raw;
        $upload_max_filesize_kb   = $upload_max_filesize_unit === 'g' ? $upload_max_filesize_num * 1024 * 1024
            : ($upload_max_filesize_unit === 'm' ? $upload_max_filesize_num * 1024
            : ($upload_max_filesize_unit === 'k' ? $upload_max_filesize_num
            : $upload_max_filesize_num / 1024));
        ?>
...
<?php if (isset($page_name) && $page_name == 'courses-server-side') : ?>
    <script type="text/javascript">
