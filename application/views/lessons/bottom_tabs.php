...
<ul class="nav nav-tabs ct-tabs-custom-one player-bottom-tabs mt-3" role="tablist">
	...
	<?php if (isset($announcements) && count($announcements) > 0): ?>
		<li class="nav-item" role="presentation">	
			<button class="nav-link" id="announcement-tab" data-bs-toggle="tab"	data-bs-target="#announcement-content" type="button" role="tab" aria-controls="announcement-content" aria-selected="true">
						...
						<p class="text-start">	    			    
							<b>
								<?php if ($announcement['is_pinned']): ?>
									<span class="badge bg-warning text-dark mr-2">
										<i class="fa fa-thumbtack"></i>
										<?php echo get_phrase('Pinned'); ?>
									</span>
								<?php endif; ?>
								<?php echo html_escape($announcement['title']); ?>
							</b>	    			    
							<small class="pl-3 text-muted text-12 float-end"><?php echo date('d M Y', strtotime($announcement['created_at'])); ?></small>
						</p>                 

						<hr>

						<div>
							<?php echo htmlspecialchars_decode($announcement['description']); ?>
						</div>

						<?php if (!empty($announcement['attachment'])): ?>

							<hr>
							<a href="<?php echo site_url('home/download_announcement_file/'.$announcement['attachment']); ?>" class="btn btn_zoom">
								<i class="fa fa-download"></i>
								<?php echo get_phrase('Download_attachment'); ?>
							</a>
