...
<div class="row ">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                <h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i> <?php echo get_phrase('Home page builder'); ?></h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
...
<!--
    Which front end the site root serves. Without this control the Home Page
    Builder below configures a page that nobody reaches, because the root
    redirects to the bilingual academy site instead.
-->
<div class="card">
    <div class="card-body">
        <h4 class="page-title mb-3"><i class="mdi mdi-home-search title_icon"></i> <?php echo get_phrase('What the site root shows'); ?></h4>
        <?php $root_frontend = get_frontend_settings('root_frontend'); ?>
        <?php if ($root_frontend !== 'lms'): ?>
            <div class="alert alert-warning mb-3">
                <strong><?php echo get_phrase('Heads up'); ?>:</strong>
                <?php echo get_phrase('The site root currently shows the bilingual academy website, so the home page you build below and the theme you activate are only visible at'); ?>
                <a href="<?php echo site_url('home'); ?>" target="_blank"><?php echo site_url('home'); ?></a>.
                <?php echo get_phrase('Switch the root below if you want the built home page to be the first thing visitors see'); ?>.
            </div>
        <?php endif; ?>
        <form action="<?php echo site_url('admin/root_frontend'); ?>" method="post" class="d-flex flex-wrap align-items-end" style="gap:1rem">
            <div>
                <label class="form-label" for="root_frontend"><?php echo get_phrase('Front end shown at the site root'); ?></label>
                <select class="form-control" name="root_frontend" id="root_frontend" style="min-width:320px">
                    <option value="academy" <?php if ($root_frontend !== 'lms') echo 'selected'; ?>>
                        <?php echo get_phrase('Academy website, bilingual Arabic and English'); ?>
                    </option>
                    <option value="lms" <?php if ($root_frontend === 'lms') echo 'selected'; ?>>
                        <?php echo get_phrase('Academy LMS theme, built with the Home Page Builder'); ?>
                    </option>
                </select>
            </div>
