...
<?php if ($get_lesson_type == 'youtube_video_url') : ?>
	<style>
		/* The iframe carried a height attribute and no width, so it fell back
		   to the 300px intrinsic default and sat in a narrow column beside a
		   large empty area. The class this is keyed off is on the iframe
		   itself rather than on the wrapper, because Plyr rewrites the
		   wrapper's classes when it initialises and leaves it bare when it
		   does not, so a wrapper selector matches only sometimes. */
		.ha-lesson-embed { display: block; width: 100%; height: auto; aspect-ratio: 16 / 9; border: 0; background: #000; }
	</style>
	<div class="<?php if ($full_page) echo 'bg-black'; ?>">
		<div class="plyr__video-embed " id="player">
			<iframe class="ha-lesson-embed" height="500" src="<?php echo $lesson_details['video_url']; ?>?origin=https://plyr.io&amp;iv_load_policy=3&amp;modestbranding=1&amp;playsinline=1&amp;showinfo=0&amp;rel=0&amp;enablejsapi=1" allowfullscreen allowtransparency allow="autoplay"></iframe>
		</div>
	</div>
